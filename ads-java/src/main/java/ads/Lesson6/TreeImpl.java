...
public class TreeImpl<E extends Comparable<? super E>> implements Tree<E> {
    ...
    @Override
    public void traverse(String mode) {
//        switch (mode) { // пишет, что то не работает в java 8
//            case PRE_ORDER -> preOrder(root); //прямой
//            case IN_ORDER -> inOrder(root); //центированный
//            case POST_ORDER -> postOrder(root); //обратный
//        }
        if(mode.equals("preOrder")) preOrder(root);
        if(mode.equals("inOrder")) inOrder(root);
        if(mode.equals("postOrder")) postOrder(root);
        System.out.println();
    }

    public boolean printBalanced(){
